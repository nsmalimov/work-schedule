...
class Scheduler:
    ...
    def devide_max4(self, full_free_slots):
        res = {}

        for slot_mask in full_free_slots:
    ...
    async def get_available_free_slots(self, full_free_slots_max4):
        free_tasks = await self.data_processor.get_free_tasks()
        free_tasks_object = self.create_time_masks_from_task_objects(free_tasks)

        for i in full_free_slots_max4:
            # check slot

            # во сколько пустых слотов таск может войти
            # есть только в 1, его убираем из пула, убираем и часть слота под него
            # расщепление слота под разные варианты времени

            print(i)

        return full_free_slots_max4
    ...
    async def find_available_slots(self):
        ...
        full_free_slots = self.get_full_free_slots(workers_with_tasks)

        # из общего времени (с 10 до 17) делаем время от 1 до 4 часов в слот
        full_free_slots_max4 = self.devide_max4(full_free_slots)

        # удаляем не доступные слоты с учётом "не взятых" тасков
        available_free_slots = await self.get_available_free_slots(full_free_slots_max4)
